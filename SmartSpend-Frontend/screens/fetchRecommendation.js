...
const AI_URL = (Constants.expoConfig?.extra?.apiUrl || 'https://0da41aa167cb.ngrok-free.app').replace(/\/+$/, '');
...
function postProcessRecommendation({ rawRec, income, categories, age }) {
  ...
  // Floors for present categories
  const floors = {
    healthcare: 0.05, // 5%
    education: 0.03,  // bump as needed
  };
  ...
  // Seed missing present categories to 0
  [
    'Food','Transport','Utilities','Savings','Entertainment',
    'Clothing','Healthcare','Education','Emergency','Buffer',
  ].forEach((n) => {
    if (names.has(norm(n)) && out[n] === undefined) out[n] = 0;
  });

  // If over income, trim flex first then proportionally
  ...
  // Sweep leftover to Savings/Emergency/Buffer (only if user has them)
  ...
  if (leftover > 1) {
    const want = [
      ...
      { name: 'Buffer', w: 0.2 },
    ].filter(({ name }) => names.has(norm(name)));
...
export const fetchRecommendation = async () => {
  try {
    ...
    const res = await axios.post(`${AI_URL}/recommend`, { age, income, categories: categoryNames }, { timeout: 15000 });
    ...
    const rec = postProcessRecommendation({ rawRec: recRaw, income, categories: categoryNames, age });
